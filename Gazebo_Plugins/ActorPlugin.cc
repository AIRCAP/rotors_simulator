...
void ActorPlugin::Reset()
{
  this->velocity = 10;
  this->lastUpdate = 0;
  this->start = this->world->GetSimTime().Double();
...
{
  duration = ( this->world->GetSimTime().Double() - this->start ) ;
...
double ActorPlugin::heightMapZ(double x, double y)  {
   ...
   physics::ModelPtr model = this->world->GetModel("yosemite");
   ...
   math::Vector3 size = heightmap->Size();
